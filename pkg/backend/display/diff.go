...
func renderDiff(
	...
	if metadata.DetailedDiff != nil {
		...
		if diff := translateDetailedDiff(metadata); diff != nil {
			engine.PrintObjectDiff(&buf, *diff, nil /*include*/, planning, indent, opts.SummaryDiff, debug)
		} else {
			engine.PrintObject(
				&buf, metadata.Old.Inputs, planning, indent, deploy.OpSame, true /*prefix*/, debug)
		}
		details = buf.String()
	} else {
